...
def compute_reward(state, ctrl_agent_index, positions,  distance_dict, our_turn, count_down, step_reward):
    ...
    if state[ctrl_agent_index]['release']:
        ...
        if our_turn:
            ...
            if len(distance_dict["enemy"]) != 0:
                step_reward = (compute_group_distance([300, 500], distance_dict["enemy"], positions)/len(distance_dict["enemy"])
                                - compute_group_distance([300, 500], distance_dict["our_turn"], positions)/len(distance_dict["our_turn"]))/100
                #step_reward = step_reward*0.5 + 0.5*(150 - compute_distance([300, 500], positions[-1]))/100
            else: step_reward = (150 - compute_distance([300, 500], positions[-1]))/100
        else:
            ...
            if len(distance_dict["our_turn"]) != 0:
                step_reward = (compute_group_distance([300, 500], distance_dict["our_turn"], positions) /len(distance_dict["our_turn"])
                                - compute_group_distance([300, 500], distance_dict["enemy"], positions)/len(distance_dict["enemy"]))/100
                #step_reward = step_reward*0.5 + 0.5*(150 - compute_distance([300, 500], positions[-1]))/100
            else: step_reward = (150 - compute_distance([300, 500], positions[-1]))/100
        ...
        step_reward = 0
        if count_down < 45:
            step_reward = -0.01#-(min(0.05 * (50-count_down), 1))
        if our_turn:
